...
namespace DatabaseHelper
{
    public class DatabaseHelper
    {
        ...
        {
            MySqlCommand cmd1 = new MySqlCommand("INSERT INTO sequence (name,description) VALUES (@name,@description)", this.conn);
            cmd1.Prepare();
        ...
        public void writeImageDataToDB(Int16[] data, int depth, int width, int height, int cameraID, int runID, int seqID)  //Modified. Factor of 2 because int 16 to int 8
        {


            MySqlCommand cmd1 = new MySqlCommand("INSERT INTO images (runID_fk,sequenceID_fk,cameraID_fk,data, type) VALUES(@runID,@sequenceID,@cameraID,@data, @type)", this.conn);
            cmd1.Prepare();


            byte[] bytesData = new byte[2 * depth * height * width];


            Buffer.BlockCopy(data, 0, bytesData, 0, bytesData.Length);


            cmd1.Parameters.AddWithValue("@data", bytesData);
            cmd1.Parameters.AddWithValue("@runID", runID);
            cmd1.Parameters.AddWithValue("@sequenceID", seqID);
            cmd1.Parameters.AddWithValue("@cameraID", cameraID);
            cmd1.Parameters.AddWithValue("@type", "temp");

            cmd1.ExecuteNonQuery();
        }

        public void writeImageDataToCache(Int16[] data, int depth, int width, int height, int cameraID, int runID, int seqID)
        {
            ...
            client.Store(Enyim.Caching.Memcached.StoreMode.Set, "data", bytesData);
            client.Store(Enyim.Caching.Memcached.StoreMode.Set, "camID", cameraID);
            client.Store(Enyim.Caching.Memcached.StoreMode.Set, "runID", runID);
            client.Store(Enyim.Caching.Memcached.StoreMode.Set, "seqID", seqID);
        ...
        public void writeCameraDataToDB(float pixelsize, int depth, int width, int height)
        {
            MySqlCommand cmd1 = new MySqlCommand("INSERT INTO cameras (cameraHeight,cameraWidth,depth,cameraPixelSize) VALUES(@height,@width,@depth,@pixelsize)", this.conn);
            cmd1.Prepare();

            cmd1.Parameters.AddWithValue("@height", height);
            cmd1.Parameters.AddWithValue("@width", width);
            cmd1.Parameters.AddWithValue("@depth", depth);
            cmd1.Parameters.AddWithValue("@pixelsize", pixelsize);
        ...
        public imageStruct getCameraData(int cameraID) //Used by camera slave to show camera data
        {
            imageStruct outputStruct = new imageStruct();

            MySqlCommand cmd1 = new MySqlCommand("SELECT cameraHeight,cameraWidth, Depth, cameraPixelSize FROM cameras WHERE cameraID = @ID", this.conn);
            cmd1.Prepare();
            ...
            while (reader2.Read())
            {
                ...
                width = (int)reader2[1];
                depth = (int)reader2[2];
                pixelSize = Convert.ToDouble(reader2[3]);
            }
            reader2.Close();
        ...
        public double[] getVariableValuesFromInputValuesTable() //what is input values table?
        {
            MySqlCommand cmd1 = new MySqlCommand("SELECT * FROM InputValues", this.conn); //get list of columns
            MySqlDataReader reader = cmd1.ExecuteReader(); //Reading the columns
        ...
        //{
        //    MySqlCommand cmd = new MySqlCommand("UPDATE analysisResults set nBEC=" + Convert.ToString(nBEC), connectionToGADatabase);
        //    cmd.ExecuteNonQuery();
        //    cmd = new MySqlCommand("UPDATE analysisResults set updated = 1", connectionToGADatabase);
        //    cmd.ExecuteNonQuery();
        //}
        ...
        */
        ...
        public freeImageStruct readFreeImageFromCache() //Returns the 3 arrays, camera info, cicero variables. but draws the 3 arrays from the cache
        {
            ...
            Buffer.BlockCopy(shot, 0, shotBytes, 0, shotBytes.Length);
        ...
        public imageStruct readImageFromCache() //Returns the 3 arrays, camera info, cicero variables. but draws the 3 arrays from the cache
        {
            ...
            int depth = new int();
            double pixelSize = new double();
            byte[] dataBytes = (byte[])client.Get("data");
            ...
            while (reader.Read())
            {
                ...
                width = (int)reader[1];
                depth = (int)reader[2];
                pixelSize = Convert.ToDouble(reader[3]);
            }
            reader.Close();

            Int16[] data = new Int16[2 * depth * height * width];

            Buffer.BlockCopy(dataBytes, 0, data, 0, data.Length);
        ...
        public imageStruct readImage() //(OVERLOADED) Returns the three arrays, camera info, and all cicero variables from the last run
        {
            ...
            MySqlCommand cmd1 = new MySqlCommand("SELECT data, cameraID_fk,runID_fk,sequenceID_fk, timestamp FROM images ORDER BY imageID DESC LIMIT 1", this.conn);
            cmd1.Prepare();
            imageStruct outputStruct = new imageStruct();
            MySqlDataReader reader = cmd1.ExecuteReader(); //Reading the columns
            ...
            while (reader.Read())
            {
                ...
                dataBytes = (byte[])reader[0];
                runID = (int)reader[2];
                seqID = (int)reader[3];
                timestamp = reader[4].ToString();
                cmd1 = new MySqlCommand("SELECT cameraHeight,cameraWidth, Depth, cameraPixelSize FROM cameras WHERE cameraID = @ID", this.conn);
                cmd1.Prepare();
                cmd1.Parameters.AddWithValue("@ID", (int)reader[1]);
            }
            reader.Close();
            MySqlDataReader reader2 = cmd1.ExecuteReader();
        ...
        {
            MySqlCommand cmd1 = new MySqlCommand("SELECT data, cameraID_fk,runID_fk,sequenceID_fk, timestamp FROM images WHERE imageID = @id", this.conn);
            cmd1.Prepare();
        ...
        {
            MySqlCommand cmd1 = new MySqlCommand("SELECT data, cameraID_fk, imageID, timestamp FROM images ORDER BY imageID DESC LIMIT @N", this.conn);
            cmd1.Prepare();
        ...
        {
            MySqlCommand cmd1 = new MySqlCommand("SELECT data, cameraID_fk, imageID, timestamp FROM images WHERE sequenceID_fk = @seqID", this.conn);
            cmd1.Prepare();
        ...
        //Functions for populating lists in the analysis gui/data viewer (BEC 5)
        ...
        public string[] getLastNImageTimestamps(int N)
        {
            ...
            while (reader.Read())
            {
                if (reader[0] != DBNull.Value) stamps[k] = reader[0].ToString();
                else stamps[k] = "None";
                k++;
            }
        ...
        public object readLastSequenceNameAndDescription()
        {
            MySqlCommand cmd1 = new MySqlCommand("SELECT name,description FROM sequence ORDER BY sequenceID DESC LIMIT 1", this.conn);
            cmd1.Prepare();
            MySqlDataReader reader = cmd1.ExecuteReader();
            description output = new description();
